...
panes computes layout in f32 continuous space. Each renderer has its own coordinate system. Adapter crates bridge the gap so you write the layout once and render it anywhere.

| Adapter | Target | Coordinate system | What the adapter does |
|---------|--------|-------------------|----------------------|
| `panes-ratatui` | Terminal (ratatui) | u16 integer cells | Quantizes f32→u16 with edge-rounding. No gaps, no overlaps. |
| `panes-egui` | Desktop/mobile (egui) | f32 logical pixels | Direct pass-through. No conversion needed. |
| `panes-css` | Browser | CSS declarations | Emits flexbox/grid CSS. The browser solves layout. |
| `panes-wasm` | Canvas/JS | f64 JS numbers | Widens f32→f64 for JavaScript's number model. |

### Write once, render anywhere

The layout definition is the same regardless of target:

```rust
let layout = Layout::master_stack(["editor", "chat", "status"])
    .master_ratio(0.6)
    .gap(1.0)
    .build()?;
```

Each adapter resolves and converts for its coordinate system:

```rust
// Terminal — quantized u16 cells
let frame = panes_ratatui::resolve_layout(&layout, terminal_area)?;

// egui — f32 pixels (direct)
let resolved = layout.resolve(available.width(), available.height())?;
for entry in panes_egui::panels(&resolved) { /* f32 rect */ }

// CSS — no resolve, just emit
let css = panes_css::emit(&layout);

// WASM — f64 for JS
let resolved = layout.resolve(width, height)?;
for entry in panes_wasm::panels(&resolved) { /* f64 rect */ }
```

### Adapter APIs

Each adapter provides these standard functions (generated by `impl_adapter!`):
- `convert()` — `FxHashMap<PanelId, TargetRect>` for random access
- `panels()` — lazy iterator of `PanelEntry { id, kind, rect, kind_index }` for rendering loops
- `overlays()` — lazy iterator of `OverlayEntry { id, kind, rect }` for overlay rendering
